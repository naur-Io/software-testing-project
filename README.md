...
1. Comece pelos casos felizes (checkout básico sem cupom).
2. Adicione casos de borda:

   * carrinho vazio,
   * estoque insuficiente,
   * cupom inválido/expirado/já usado,
   * frete por peso,
   * promoções de frete grátis,
   * combos de cupom + pagamento (incluindo restrições de acumulação),
   * imposto mínimo/zero,
   * etc...

3. Só depois, _persiga_ mutantes sobreviventes, sempre perguntando:

   * "_Que regra eu não testei?_" ou
   * "_Meu assert está fraco?_"
...
1. Rode:
   ...
   ./gradlew clean
   ```
2. Compacte o projeto em um arquivo `.zip` (após o `clean` para reduzir tamanho).
3. Inclua um arquivo `RELATORIO.md` (curto) com:

   * resumo dos cenários cobertos;
   * relato do processo de escrita dos testes;
      * inclua observações e discussões sobre quais regras foram mais desafiadoras, o que foi mais difícil de testar, o que aprendeu no processo, etc.

Anexe o `.zip` no Google Classroom.